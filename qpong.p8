...
function update_ball()
    ...
    -- collide with court
    if nget_float("ball_y_pos") + ball_width >= court.bottom - 1
    or nget_float("ball_y_pos") <= court.top + 1 then
    ...
    if nget("ball_x_spd_dir") == 0
     ...
     then
        new_dy = nget_float("ball_y_spd") - 1.7 * ball_speedup
        ...
        --nget("ball_x_spd", -(nget("ball_x_spd") - 2.0 * ball_speedup))
        --nset("ball_x_spd_dir", (nget("ball_x_spd_dir") + 1) % 2)
        --nset("ball_y_spd_dir", (nget("ball_y_spd_dir") + 1) % 2)
        sfx(1)
     end
    ...
    -- ball movement
    dir_x = nget("ball_x_spd_dir")
    dir_y = nget("ball_y_spd_dir")

    new_dx = 0
    new_dy = 0

    if dir_x == 0 then
        new_dx = nget_float("ball_x_pos") - nget_float("ball_x_spd")
    else
        new_dx = nget_float("ball_x_pos") + nget_float("ball_x_spd")
    end

    if dir_y == 0 then
        new_dy = nget_float("ball_y_pos") - nget_float("ball_y_spd")
    else
        new_dy = nget_float("ball_y_pos") + nget_float("ball_y_spd")
    end
...
function reset_ball()
    ...
    nset_float("ball_y_pos", 33 + rnd(5))
    nset_float("ball_x_spd", 0.7)
